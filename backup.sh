...
pg_dump_database() {
    pg_dump  --no-owner --no-privileges --clean --if-exists --quote-all-identifiers --encoding=ISO88591 "$DATABASE_URL"
}
...
upload_to_bucket() {
    ...
    s3 cp - "s3://$AWS_DB_BACKUPS_BUCKET_NAME/$AWS_DB_BACKUPS_FILENAME"
}
